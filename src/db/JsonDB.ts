...
export class JsonDB {
    ...
    private retrieveData(dataPath: DataPath, create = false): any {
        this.load()
    ...
    private getArrayData(dataPath: string): any {
        const result = this.getData(dataPath)
        if (!Array.isArray(result)) {
            throw new DataError(`DataPath: ${dataPath} is not an array.`, 11)
        }
     ...
     */
    public push(dataPath: string, data: any, override = true): void {
    ...
    public load(): void {
        ...
        try {
            this.data = this.config.adapter.read();
            this.loaded = true
        } catch (err: any) {
    ...
    public save(force?: boolean): void {
        ...
        try {
            this.config.adapter.write(this.data);
        } catch (err: any) {
